...
class HiveEncoder(nn.Module):
    ...
    def forward(self, rnn_states_in, *processed_obs):
        ...
        flat_in  = torch.cat([processed_obs[0], cmd_prev], dim=1)

        # Run ant comm block
        logits, _, new_a_state = self.ant_comm_block(
            self._split_state(rnn_states_in)[0],
            flat_in, processed_obs[1], processed_obs[2])
    ...
    def fwd_sequence(self, rnn_start_states, sequence_breaks,
        ...
        # cmds_prev = self._compute_cmd_prev(rnn_start_states)
        # flat_in  = torch.cat([flattened_inputs[0], cmds_prev], dim=1)

        # logits, _, new_a_state = self.ant_comm_block(
        #     self._split_state(rnn_start_states)[0],
        #     flat_in, flattened_inputs[1], flattened_inputs[2])

        # logits_seq = logits.view(
        #     *sequence_breaks.shape[0:2], *logits.shape[1:])

        # rnn_state_seq = new_a_state.view(2, 1,
        #     *sequence_breaks.shape[0:2], *new_a_state.shape[3:])

        # return logits_seq, rnn_state_seq

        # Reshape flattened_processed_obs to [seq len, num of seqs, obs dim]
        flattened_processed_obs_seq = flattened_processed_obs.view(
            *sequence_breaks.shape[0:2], *flattened_processed_obs.shape[1:])
