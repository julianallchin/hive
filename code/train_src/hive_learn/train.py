...
def _gather_minibatch(rollouts : Rollouts,
    ...
    rewards_slice = _mb_slice(rollouts.rewards, inds).to(
        dtype=amp.compute_dtype)
    active_agents_slice = _mb_slice(rollouts.active_agents, inds)
    ...
    return MiniBatch(
        ...
        dones=dones_slice,
        active_agents=active_agents_slice,
...
def _ppo_update(cfg : TrainConfig,
    ...
    with amp.enable():
        ...
        with torch.no_grad():
            ...
            weight = mb.active_agents.to(dtype=amp.compute_dtype)
        ...
        action_obj = torch.mean(action_obj * weight)
        value_loss = torch.mean(value_loss * weight)
